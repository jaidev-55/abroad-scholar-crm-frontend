...
// ─── CSV builder ──────────────────────────────────────────────────────────────
const buildCSV = (leads: Lead[], fields: string[]): string => {
  ...
  const rows = leads.map((lead) =>
    activeFields.map((f) => {
      ...
      if (f.key === "notesText") {
        ...
        val = String((lead as Record<string, unknown>)[f.key] ?? "");
      }
...
interface ExportModalProps {
  leads: Lead[]; // filtered leads
  allLeads: Lead[]; // all leads
  onClose: () => void;
}
