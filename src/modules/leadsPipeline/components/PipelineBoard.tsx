...
// Converted & lost columns only show today's activity — all other stages show everything
const getLeadsForStage = (leads: Lead[], stageId: string): Lead[] => {
  ...
  if (stageId === "converted" || stageId === "lost") {
    // Fall back to createdAt when updatedAt is missing
    return stageLeads.filter((l) => isTodayDate(l.updatedAt ?? l.createdAt));
  }
...
const PipelineBoard: React.FC<PipelineBoardProps> = ({
  ...
  //  card being dragged — passed to DragOverlay for the ghost preview
  ...
  return (
    ...
    >
      ...
      {/* Ghost card rendered at the cursor position while dragging */}
      <DragOverlay>
        {activeLead ? (
          <KanbanCard
            lead={activeLead}
